 ...
 *
 * Get MEM loads near (close to) a specified range of timestamps.
 * The timestampts range from 'timestamp' to timestamp+step*(count-1).
 */
static void
nearest_loads (const MEMGraph *base, guint core, gint64 start, gint64 step, gssize count, gfloat *out)
{
    const gssize history_cap_pow2 = base->history.cap_pow2;
    const MemLoad *history_data = base->history.data[core];
    ...
    if (!base->non_linear)
    {
        ...
        for (i = 0, j = 0; i < count; i++)
        {
            ...
            {
                MemLoad load = history_data[(history_offset + j) & history_mask];
    ...
    else
    {
        ...
        for (i = 0; i < count; i++)
        {
             ...
             *  - Both timestamps are pointing to a time before the MEM load measurements have started
...
void
draw_graph_normal (MEMGraph *base, cairo_t *cr, gint w, gint h, guint core)
{
    gint x, y;
    gint tmp;
...
void
draw_graph_LED (MEMGraph *base, cairo_t *cr, gint w, gint h, guint core)
{
...
void
draw_graph_no_history (MEMGraph *base, cairo_t *cr, gint w, gint h, guint core)
{
    gfloat usage;
...
void
draw_graph_grid (MEMGraph *base, cairo_t *cr, gint w, gint h, guint core)
{
